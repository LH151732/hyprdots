...
## 2026-03-28 - 双便携屏上下排列 + 动态 DP 检测

### 问题
- 两个 RTK Monitor (2560x1600@240Hz) 的 description/serial 完全相同
- DP-x 编号每次重启/重连都会变化, 无法写死配置

### 方案
启动脚本动态检测 DP 编号, 写入配置文件, 再 reload 生效.

### 修改文件
1. **`~/.config/hypr/monitors.conf`** - 添加 source 动态配置 + 通配符 fallback
2. **`~/.config/hypr/monitors_dynamic.conf`** - 脚本自动生成, 包含当前 DP 编号和上下位置

### 新增文件
1. **`~/.local/share/bin/monitor_stack.sh`** - 检测 DP 显示器, 生成上下排列配置并 reload

### 配置参数
- 缩放: 1.6x (有效分辨率 1600x1000)
- 布局: 上下垂直 (上屏 y=0, 下屏 y=1000)
- 刷新率: 240Hz
